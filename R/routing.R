#' Calculate the water balance and route downstream
#'
...
water_balance = function(neigh,state) {
  for(i in seq(1,nrow(neigh))) {
    state$overspill[i]=max(
      state$volume[i]+state$incoming_runoff[i]+sum(state$overspill[neigh$neighborhood[i][[1]]])-state$max_volume[i],
      0)
    state$volume[i]=min(state$max_volume[i],
      state$volume[i]+state$incoming_runoff[i]+sum(state$overspill[neigh$neighborhood[i][[1]]])
    )
  }
  return(state)
}
